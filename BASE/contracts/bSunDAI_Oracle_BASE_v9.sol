...
/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║       bSunDAI Oracle v9 — Hybrid Chainlink + TWAP + Warning Track    ║
 * ║                                                                      ║
 ...
 * ║   If sustained for 30 minutes → getLiquidationPrice() switches to    ║
 * ║   live Chainlink for liquidation eligibility, closing the bad-debt   ║
 * ║   window that conservative committed-price stepping inherently       ║
 * ║   creates during real crashes.                                       ║
 * ║   Flash crashes: Chainlink recovers in minutes, warning clears.      ║
 * ║   Real crashes: Chainlink stays down 30+ min, liquidations proceed.  ║
 * ║                                                                      ║
 * ║   v9 CHANGES FROM v7 (renamed to stay version-aligned with           ║
 * ║   pSunDAI V9 on PulseChain):                                         ║
 * ║   ✓ NEW: getLiquidationPrice() — the live Chainlink price used for   ║
 * ║     liquidation once the warning track confirms is now hard-clamped  ║
 * ║     to within MAX_CHAINLINK_DEVIATION_BPS (15%) of committedPrice.   ║
 * ║     v7 used raw, unclamped live Chainlink once the warning confirmed ║
 * ║     — Chainlink is far harder to manipulate in a single transaction  ║
 * ║     than an on-chain AMM spot price, but an unclamped live-price     ║
 * ║     path is still a real amplifier for extreme volatility, a         ║
 * ║     Chainlink feed glitch, or a compromised Aave oracle wrapper.     ║
 * ║     This is the exact defense-in-depth pattern pSunDAI V9 added to   ║
 * ║     its own oracle (MAX_SPOT_DEVIATION_BPS) after a confirmed        ║
 * ║     unbounded-liquidation-profit exploit was found in pSunDAI V8.    ║
 * ║     The vault now calls this single function instead of doing its    ║
 * ║     own dual-track branching — centralizes the pricing logic here.   ║
 * ║                                                                      ║
 * ║   UNCHANGED from v7:                                                 ║
 * ║   ✓ Chainlink + V3 TWAP hybrid selection logic                       ║
 * ║   ✓ USDC depeg guard                                                 ║
 * ║   ✓ 3%/10% step sizes, STEP_INTERVAL, no dead-zone stepping          ║
 * ║   ✓ Chainlink warning track trigger/confirm timing (3% / 30min)      ║
...
contract bSunDAIoracleBASE_v9 {

    string public constant VERSION = "bSunDAIoracleBASE_v9.0";
    ...
    /// @notice v9: max deviation allowed between the live Chainlink liquidation price
    ///         and committedPrice, once the warning track has confirmed. Caps how far
    ///         a liquidation price can move from the trusted conservative price no
    ///         matter what live Chainlink reports — defense-in-depth against extreme
    ///         volatility, feed glitches, or a compromised oracle wrapper. Mirrors
    ///         pSunDAI V9's MAX_SPOT_DEVIATION_BPS fix for the same class of risk.
    uint256 public constant MAX_CHAINLINK_DEVIATION_BPS = 1500; // 15%
    ...
    // When nonzero for >= CHAINLINK_CONFIRM_TIME: liquidation price switches to
    // (clamped) live Chainlink via getLiquidationPrice().
    ...
    ///         price for >= CHAINLINK_CONFIRM_TIME.
    function isChainlinkLiquidationEnabled() public view returns (bool) {
    ...
    /**
     * @notice v9: the price the vault should use for liquidation eligibility/reward.
     *         Once the Chainlink warning track has confirmed (30+ min sustained
     *         divergence), returns live Chainlink — but hard-clamped to within
     *         MAX_CHAINLINK_DEVIATION_BPS (15%) of committedPrice. Otherwise
     *         returns committedPrice, same as minting/withdrawal safety checks.
     * @return price    The (possibly clamped) liquidation price.
     * @return isLive   True if the live-Chainlink track is active (even if clamped).
     */
    function getLiquidationPrice() external view returns (uint256 price, bool isLive) {
        if (isChainlinkLiquidationEnabled()) {
            uint256 clPrice = _getChainlinkPrice();
            if (clPrice > 0 && committedPrice > 0) {
                uint256 lower = (committedPrice * (10000 - MAX_CHAINLINK_DEVIATION_BPS)) / 10000;
                uint256 upper = (committedPrice * (10000 + MAX_CHAINLINK_DEVIATION_BPS)) / 10000;
                uint256 clamped = clPrice;
                if (clamped < lower) clamped = lower;
                if (clamped > upper) clamped = upper;
                return (clamped, true);
            }
        }
        return (committedPrice, false);
    }
    ...
    /// @dev No dead zone. Any move > instant threshold starts confirmation.
    function _handleNormal(uint256 raw) internal {
        ...
        if (bps <= instantB) {
            ...
            // Larger move: always start confirmation (no dead zone)
            pendingTarget    = raw;
            inConfirmation   = true;
    ...
    /// @notice Live Chainlink price (1e18). Diagnostic — vault uses getLiquidationPrice().
