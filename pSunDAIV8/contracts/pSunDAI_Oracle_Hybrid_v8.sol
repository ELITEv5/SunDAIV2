...
/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║          pSunDAIoraclePLSXHybrid — ELITE TEAM6 (v8)                  ║
 * ║                                                                      ║
 * ║   Self-Healing Dual-Track Oracle for Autonomous Stable Assets        ║
 * ║                                                                      ║
 * ║   Carries forward all v7 logic unchanged (dual-track pricing,        ║
 * ║   spot warning, asymmetric confirmation state machine). v8 adds      ║
 * ║   three hardening mechanisms on top, all derived from real-time      ║
 * ║   on-chain pool state rather than fixed constants:                   ║
 * ║                                                                      ║
 * ║   1. maxSafeDebt() — manipulation-resistant debt capacity computed   ║
 * ║      from pool depth (20x combined valid reserves), instead of a     ║
 * ║      static ceiling picked once at deploy time. Uses min(current,    ║
 * ║      rolling-24h-minimum) depth, not the instantaneous value — a     ║
 * ║      liquidity increase only counts once sustained for the full      ║
 * ║      window, closing a flash-liquidity attack (add liquidity, mint   ║
 * ║      against the momentarily-inflated cap, withdraw the liquidity).  ║
 * ║      A genuine liquidity drop is still reflected immediately.        ║
 * ║                                                                      ║
 * ║   2. Liquidity-weighted median — both the TWAP median and the spot   ║
 * ║      median now weight each pool's vote by its stable-side reserve,  ║
 * ║      so a thin pool can no longer sway the reported price as much    ║
 * ║      as a deep one.                                                  ║
 * ║                                                                      ║
 * ║   3. Rolling-window cumulative drift cap — closes the staircase gap  ║
 * ║      where many small "instant" (sub-threshold) updates could chain  ║
 * ║      together to walk the price past what a single large move would ║
 * ║      ever be allowed to do without triggering confirmation. Samples  ║
 * ║      are spaced (ROLLING_SAMPLE_INTERVAL) so the buffer reliably     ║
 * ║      spans the full window even under sustained rapid updates.       ║
 * ║                                                                      ║
...
contract pSunDAIoraclePLSXHybrid_v8 {
    using Math for uint256;

    string public constant VERSION = "pSunDAIoraclePLSXHybrid_v8";

    // ── Pair data ────────────────────────────────────────────────────────────
    struct PairData {
        IUniswapV2Pair pair;
        uint256 priceCumulativeLast;
        uint40  oracleTimestampLast; // oracle's own last-sample time (H-1 fix, carried from v6)
        uint8   stableDecimals;      // cached at init (M-5 fix, carried from v6)
        bool    wplsIsToken0;
    }
    ...
    // ── Rolling drift-tracking state (v8) ────────────────────────────────────
    ...
    // allowed to move it without triggering the confirmation delay.
    // Samples are spaced at least ROLLING_SAMPLE_INTERVAL apart so the 8 slots
    // reliably span the full STAIRCASE_WINDOW even under sustained high-frequency
    // updates (e.g. an active staircase attempt itself) — without this spacing
    // gate, frequent updates would fill and wrap the buffer in well under 30
    // minutes, silently shrinking the real lookback exactly when it matters most.
    ...
    // ── Liquidity-depth tracking state (v8) ──────────────────────────────────
    ...
    // current value. Closes a flash-liquidity attack: without this, someone could
    // temporarily add liquidity to the oracle's pools, mint pSunDAI against the
    // momentarily-inflated cap, then remove that liquidity — the minted debt
    // would persist even after the cap dropped back to its real level. Taking
    // min(current, rolling-window-minimum) is deliberately asymmetric: a genuine
    // liquidity withdrawal is reflected immediately (current drops, safe
    // direction), but a liquidity increase only counts once it's been sustained
    // for the full LIQUIDITY_WINDOW.
    ...
    // ── Dynamic capacity constants (v8) ───────────────────────────────────────
    // maxSafeDebt() = combined valid-pool reserves * this multiplier. Manipulating
    // the median requires moving a majority (3 of 5) of the pools; a realistic
    // worst-case sustained (multi-block, arb-resistant) attack budget is on the
    // order of the pools' own depth, not a small fraction of it. 20x is a
    // deliberately conservative starting point.
    uint256 public constant SAFE_CAPACITY_MULTIPLIER = 20;
    ...
    ///         (v8). min(current, rolling-24h-minimum) of every currently-valid
    ///         pool's stable-side reserve, times SAFE_CAPACITY_MULTIPLIER. The
    ///         rolling minimum means a liquidity increase only counts once it's
    ///         been sustained for LIQUIDITY_WINDOW — closing a flash-liquidity
    ///         attack where someone temporarily adds liquidity, mints against the
    ///         momentarily-inflated cap, then withdraws it. A genuine liquidity
    ///         drop is still reflected immediately (current is always a candidate
    ///         floor), so real risk is never masked by stale history.
    ...
    function _processPriceUpdate(uint256 newPrice) internal returns (uint256, uint256) {
        ...
        // v8 staircase fix: even when the single step is small, cumulative drift
        // over the trailing STAIRCASE_WINDOW must also stay within the instant
        // threshold, otherwise many small sub-threshold steps could chain together
        // to walk the price past what a single large move would ever be allowed
        // to do without triggering the confirmation delay.
    ...
    // ── lastPrice mutation + rolling drift buffer (v8) ────────────────────────
    ...
    /// @notice Reserve-weighted median (v8). Sorts prices+weights together, then
